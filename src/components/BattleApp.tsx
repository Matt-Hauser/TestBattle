...
const HealthBar: React.FC<HealthBarProps> = ({ currentHP, originalHP }) => {
  ...
  return (
    <div style={{display: 'flex', justifyContent: 'center', textAlign: 'center'}}>
      <div style={{ backgroundColor: 'gray', height: 10, width: '50vw', display: 'flex', justifySelf:'center' }}>
        <div style={{ backgroundColor: 'green', height: 10, width: `${percentage}%` }} />
      </div>
      <div>{currentHP}/{originalHP}</div>
    </div>
  );
};
...
//   const pokemonSeed = useSelector((state: RootState) => state.pokemon.pokemonList);
  ...
  function handleAddTurn(attacker: string, defender: string, moveUsed: Move | null) {
    ...
    const newTurnRecord = new Map(Array.from(turnRecord).concat([[numTurns, newTurn]]));
  ...
  const handleMoveClick = (move: Move, source: string) => {
    ...
    const baseDamage = (((2 * level) / 5 + 2) * move.power * (attackerStats.attack / defenderStats.defense)) / 50 + 2;
    ...
    if (move.condition) {
        switch (move.condition.name) {
            case 'leech':
                if (source === 'p1') {
                    setStats2(prevState => ({...prevState, hasCondition: true, condition: 'leech'}))
                }
                if (source === 'p2') {
                    setStats1(prevState => ({...prevState, hasCondition: true, condition: 'leech'}))
                }
    ...
    switch (move.statMod) {
        case 'attack':
            const attackMod = defenderStats.attack - Math.ceil(defenderStats.attack * .1);
            if (source === 'p1') {
            ...
            break;
        case 'defense':
            const defenseMod = defenderStats.defense - Math.ceil(defenderStats.defense * .1);
            if (source === 'p1') {
    ...
    return (
      <div>
        <div>
          ...
          <div style={{position:'relative', left: '50px'}}>{pokemonSeed[2]?.name}</div>
          <div>
            <HealthBar currentHP={p2HP} originalHP={pokemonSeed[2].stats.hp} />
            <img style={{position:'relative', left: '50px'}} src={pokemonSeed[2]?.image} alt={pokemonSeed[2]?.name} />
          </div>
        </div>
        <div>
          <animated.img style={{position:'relative', right: '50px'}} src={pokemonSeed[0]?.imageBack} alt={pokemonSeed[0]?.name} /> 
          <div style={{position:'relative', right: '50px'}}>{pokemonSeed[0]?.name}</div>
          <div>
            <HealthBar currentHP={p1HP} originalHP={pokemonSeed[0].stats.hp} />
            {pokemonSeed[0]?.moves.slice(0, 4).map(move => 
              <button key={move.name} onClick={() => handleMoveClick(move, 'p1')}>{move.name}</button>
            )}
          </div>
          <div>
      {lastMoveUsed ? (
        <p>{pokemonSeed[2].name} used {lastMoveUsed.name}</p>
      ) : (
        <>
    ...
    return (
      <div>
        <div>
          ...
          <div style={{position:'relative', left: '50px'}}>{pokemonSeed[0]?.name}</div>
          <div>
            <HealthBar currentHP={p1HP} originalHP={pokemonSeed[0].stats.hp} />
            <img style={{position:'relative', left: '50px'}} src={pokemonSeed[0]?.image} alt={pokemonSeed[0]?.name} /> 
          </div>
        </div>
        <div>
          <img style={{position:'relative', right: '50px'}} src={pokemonSeed[2]?.imageBack} alt={pokemonSeed[2]?.name} /> 
          <div style={{position:'relative', right: '50px'}}>{pokemonSeed[2]?.name}</div>
          <div>
            <HealthBar currentHP={p2HP} originalHP={pokemonSeed[2].stats.hp} />
            {pokemonSeed[2]?.moves.slice(0, 4).map(move => 
              <button key={move.name} onClick={() => handleMoveClick(move, 'p2')}>{move.name}</button>
            )}
          </div>
          <div>
      {lastMoveUsed ? (
        <p>{pokemonSeed[0].name} used {lastMoveUsed.name}</p>
      ) : (
        <>
